...
class Estudiantes extends Conexion {
    ...
    public function update($Id, $Nombre, $Apellido, $Documento, $Correo, $Materia, $Docente, $Promedio, $Fecha){
        $statement = $this->db->prepare("UPDATE estudiantes SET 
            NOMBRE = :Nombre, 
            APELLIDO = :Apellido, 
            DOCUMENTO = :Documento, 
            CORREO = :Correo, 
    ...
    public function getById($Id){
        ...
        $statement = $this->db->prepare("SELECT 
            ID_ESTUDIANTE, NOMBRE, APELLIDO, DOCUMENTO, CORREO, 
    ...
    public function search($search){
        ...
        $statement = $this->db->prepare("SELECT 
            ...
            OR APELLIDO LIKE CONCAT('%', :Search, '%') 
            OR DOCUMENTO LIKE CONCAT('%', :Search, '%') 
            OR CORREO LIKE CONCAT('%', :Search, '%') 
            OR MATERIA LIKE CONCAT('%', :Search, '%') 
    ...
    public function obtenerCursosEstudiante($IdEstudiante){
        $rows = [];
        $statement = $this->db->prepare("SELECT 
            c.ID_CURSO, c.NOMBRE_CURSO, c.NIVEL 
    ...
    public function obtenerNotasEstudiante($IdEstudiante, $IdPeriodo = null){
        $query = "SELECT 
                    n.*, m.MATERIA, e.NOMBRE, e.APELLIDO, p.NOMBRE_PERIODO 
                  FROM notas n 
    ...
    public function getEstudiantesCurso($IdCurso){
        $rows = [];
        $statement = $this->db->prepare("SELECT 
            e.ID_ESTUDIANTE, e.NOMBRE, e.APELLIDO, e.DOCUMENTO 
            FROM estudiante_cursos ec 
